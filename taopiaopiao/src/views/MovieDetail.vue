  ...
  <section id="movie-detail">
    ...
    <header class="mbg pr">
      <figure class="d-cover">
        <img
          :src="detailObj.poster ? `https://gw.alicdn.com/${detailObj.poster}` : 'https://gw.alicdn.com/tps/i1/TB147JCLFXXXXc1XVXXxGsw1VXX-112-168.png'"
          alt="">
      </figure>
    ...
    <section v-if="infoObj">
      <section class="intro-block border-1px-bottom">
        <article class="d-intro">
          <p ref="dint" :class="{'hide-something':hideSomething}">{{infoObj.description}}</p>
...
import HotEvaluation from '../components/movieDetail/hotEvaluation'
    export default{
    name:'Movie-detail',
        data(){
            return{
    ...
    this.hideSomething = !this.hideSomething
        ...
        let hotList =this.$store.state.city.data
       ...
       //由于后台没有配置更多的请求接口，
        ...
        }else if(!comingLists.length && !hotList ){
        this.$router.push('/')
        }else{
                this.detailObj=this.getDataById(hotList,id)
        }
	    let detailStr = this.detailObj.detailStr
		if (detailStr) {
			this.requestData(`/movie/info/?name=${detailStr}`, (response) => {

				this.infoObj = response.data.data.data.returnValue
	    this.initSwiper()

	    this.requestData(`/movie/evaluation/?name=${detailStr}`, (response) => {
				this.evalLists = response.data.data.data.returnValue
			})
			})
    }else {
			this.infoObj =false
